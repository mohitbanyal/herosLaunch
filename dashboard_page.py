...
# this will show you the dashboard
def show_dashboard(name, username):
    st.title(f"Welcome {name.capitalize()}")
    st.sidebar.write(f'Welcome *{name}*! ([Logout](/?logout=true))')

    print(f"username : {username.upper()}_SHEET_NAME")
    #create a connection
    conn = st.connection("gsheets",type=GSheetsConnection)
    sheet_url = os.getenv("GOOGLE_SHEET_URL")
    varibale = f"{username.upper()}_SHEET_NAME"
    sheet_name = os.getenv(varibale)
    #sheet_name = "MOHITBANYAL_SHEET_NAME"

    print(f"sheet name : {sheet_name} , variable: {varibale}")
    ...
    with st.sidebar.form("entry_form", clear_on_submit=True):
        ...
        if submit:
            ...
            conn.update(spreadsheet=sheet_url, worksheet=sheet_name, data=update_df)
    ...
    df = conn.read(spreadsheet=sheet_url, worksheet=sheet_name, ttl="10m") # Cache results for 10 min
    ...
    if not df.empty:
        ...
        with col3:
            ...
            if last_Weight - second_last_weight > 0:
                symbol = "↑"
            col3.metric("Last Weight Change", f"{last_Weight - second_last_weight}{symbol}")
...
#show_dashboard("mohitbanyal", "Adiyogi")